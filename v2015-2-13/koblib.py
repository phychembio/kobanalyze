...
def initialize(q):
    ...
    if q.needmolID: q.molL=readmolID(q) 
...
def gettrajinfo(q):
    ...
    q.zlo=float(info[0])
    q.zhi=float(info[1])
    q.xboxlength=q.xhi-q.xlo
    q.yboxlength=q.yhi-q.ylo
    q.zboxlength=q.zhi-q.zlo
    q.boxlength=q.xboxlength
    trajf.next() #ITEM: ATOMS id type....
    maxtypeid=0
    for line in range(q.noofatom):
        line=trajf.next()
        typeid=int(line.split()[1])
        if typeid>maxtypeid:
            maxtypeid=typeid    
...
def inputstate(trajf,q,timestep):
    ...
    for i in range(0,q.noofatom):
        ...
        if xorxs=="x":
            ...
            x=float(atomdata[3])*q.xboxlength-q.xlo
            y=float(atomdata[4])*q.yboxlength-q.ylo
            z=float(atomdata[5])*q.zboxlength-q.zlo          
...
def insertCEC( state, evbf, waterorderL,CECorderL,CECtimestep,q):          
 ...
 #---------------------------           
            if q.needmolID:
               ...
               for item in tempOW_L:
                   waterorderL.append([item[0],item[1]])  #stores (atomid, mod ID)
...
#------------------------------------           
                    if q.needmolID: #swap the current hydronium to water for next step                        
                        for i in range(0,len(waterorderL)):
                           if waterorderL[i][1]==pivotL[pivotL_index]:
                                # print("%d -->%d"%(nextwaterorderL[i][1],q.nextorderL[whichCEC][0]))
                               
                                 waterorderL[i][1]= CECorderL[whichCEC][0]                                 
                                 waterorderL[i][0]= q.molL[waterorderL[i][1]-q.molL[0][1]][0]
                                 break                         
#--------------------------------
           
                    CECorderL[whichCEC][0]=pivotL[pivotL_index] #change mol_id for next CEC      
...
def printresults(suffix,q):
    ...
    if q.interface:
        ...
        for type in q.iontype:
            L=q.bigionL[count]
            outf=open('Ion_type='+str(type)+suffix+'.txt','w')
            for bin in L[1:]:
                outf.write("%f   %f \n" %(bin[0] ,bin[1]*1./q.bigionL[0]))
            outf.close()
            count+=1